  ...
  data () {
    return {
      ...
      options: {
        ...
        title: {
          display: true,
          text: 'Number of cars in car park',
          fontSize: 20
        }
      }
    }
    ...
    async loadData () {
      try {
        ...
        Object.keys(lots).forEach(element => {
          lots[element] = lots[element].sort((a, b) => (moment(a.time.toDate()).valueOf() - moment(b.time.toDate().valueOf()))) // Sort time in each deviceID
        })
        Object.keys(lots).forEach(element => {
